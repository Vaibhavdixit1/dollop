        ...
        <header className="fixed top-0 left-0 right-0 z-50 border-b border-black/5 bg-neutral-200/80 backdrop-blur-xl transition-all duration-300">
            <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6 lg:px-12">
                <Link href="/" className="group flex items-center gap-3 transition-all active:scale-95">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-neutral-900 text-white shadow-xl transition-transform group-hover:rotate-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    >
                        <div className="relative h-6 w-6">
                            <span className={`absolute left-0 top-1/2 h-0.5 w-6 -translate-y-2 transition-all duration-300 bg-black ${isMenuOpen ? 'rotate-45 translate-y-0' : ''}`}></span>
                            <span className={`absolute left-0 top-1/2 h-0.5 w-6 -translate-y-0 transition-all duration-300 bg-black ${isMenuOpen ? 'opacity-0' : ''}`}></span>
                            <span className={`absolute left-0 top-1/2 h-0.5 w-6 translate-y-2 transition-all duration-300 bg-black ${isMenuOpen ? '-rotate-45 translate-y-0' : ''}`}></span>
                        </div>
                    </button>
                </div>
            ...
            <div className={`fixed inset-0 top-0 z-40 bg-neutral-200/98 backdrop-blur-2xl transition-all duration-500 md:hidden ${isMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
                <nav className="flex flex-col items-center justify-center h-full gap-8 p-10">
                    ...
                    <div className="mt-8 flex flex-col w-full gap-4">
                        <button className="w-full rounded-2xl bg-neutral-900 p-6 text-xl font-bold uppercase tracking-widest text-white shadow-2xl">
                            Book Now
                        </button>
                        <button onClick={() => setIsMenuOpen(false)} className="text-neutral-900 font-bold uppercase tracking-widest opacity-60">Close</button>
                    </div>
                </nav>
            </div>
        </header>
