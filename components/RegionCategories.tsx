        ...
        <section className="bg-white py-32 px-6 lg:px-12">
            <div className="mx-auto max-w-7xl">
                <div className="flex flex-col items-center text-center">
                    <span className="text-xs font-black tracking-[0.4em] text-neutral-400 uppercase">Explore by Type</span>
                    <h2 className="mt-8 text-6xl font-black tracking-tighter text-neutral-900 uppercase">WHERE DO YOU <br /> BELONG?</h2>
                </div>

                <div className="mt-24 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    {categories.map((cat) => (
                        <div key={cat.title} className="group relative aspect-[3/4] overflow-hidden rounded-[40px] cursor-pointer">
                            <img
                                src={cat.image}
                                alt={cat.title}
                            ...
                            <div className="absolute bottom-10 left-10 right-10">
                                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/60">{cat.count}</span>
                                <h3 className="mt-2 text-2xl font-black text-white uppercase leading-none">{cat.title}</h3>
                            </div>
                        </div>
                    ))}
                </div>
